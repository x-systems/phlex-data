...
class Field implements Expressionable
{
    ...
    public function normalize($value)
    {
        ...
        $this->assertRequiredProvided($value);
    ...
    {
        $value === null ? $this->get() : $this->normalize($value);
    ...
    {
        $this->assertValidity($value !== null && !is_scalar($value), 'Must use scalar value');
    }

    protected function assertRequiredProvided($value): void
    {
        // NULL value is always fine if it is allowed
        if ($value === null || $value === '') {
            if ($this->required) {
                throw new ValidationException([$this->name => 'Must not be null or empty'], $this->getOwner());
            }
        }
    }

    protected function assertValidity(bool $condition, string $message): void
    {
        if (!$condition) {
            throw new ValidationException([$this->name => $message], $this->getOwner());
        }
    }
