...
class DeepCopyTest extends Sql\TestCase
{
    ...
    public function testBasic(): void
    {
        ...
        $this->assertSame('John', $invoice->getTheirEntity('client')->get('name'));

        // now to add payment for the invoice. Payment originates from the same client as noted on the invoice
        $invoice->getTheirEntity('Payments')->insert(['amount' => $invoice->get('total') - 5, 'client_id' => $invoice->get('client_id')]);
        ...
        $this->assertSame('John', $invoice_copy->getTheirEntity('client')->get('name'));

        // finally, the client_id used for newly created payment and new invoice correspond
        $this->assertSame($invoice_copy->get('client_id'), $invoice_copy->getTheirEntity('Payments')->loadAny()->get('client_id'));
        ...
        $this->assertEquals(1, $client3->getTheirEntity('Invoices')->getCount());
        $this->assertEquals(1, $client3->getTheirEntity('Quotes')->getCount());
        $this->assertEquals(1, $client3->getTheirEntity('Payments')->getCount());
        ...
        $this->assertEquals(90, $client3->getTheirEntity('Quotes')->toQuery()->aggregate('sum', 'total')->getOne());

        // The total of the invoice we copied, should remain, it's calculated based on lines
        $this->assertEquals(108.9, $client3->getTheirEntity('Invoices')->toQuery()->aggregate('sum', 'total')->getOne());

        // Payments by this clients should also be copied correctly
        $this->assertEquals(103.9, $client3->getTheirEntity('Payments')->toQuery()->aggregate('sum', 'amount')->getOne());

        // If copied payments are properly allocated against copied invoices, then due amount will be 5
        $this->assertEquals(5, $client3->getTheirEntity('Invoices')->toQuery()->aggregate('sum', 'due')->getOne());
    }
